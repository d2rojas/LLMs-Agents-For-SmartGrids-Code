...
# Split-tool ("before" = original modify_load tool set; "after" = set_active_load/set_load
# in its place) appendix before-and-after comparison, pfagent and react_nogate, three
# models (2026-09-22). Rendered by benchmarks/build_split_tool_appendix.py:
...
# fragments, not one comparison table. "before" duplicates SOURCES' own paths rather than
# aliasing them so this table's source list stays self-contained and readable without
# cross-referencing SOURCES.
#
# gpt-4o-mini's and sol's "after" point at the 2026-09-21 split-tool launch
# (results_validation_split_tool_launch), NOT results_validation_load_split -- the editor
# found the two disagree (load_split's gpt-4o-mini PFAgent: 97.5 formulation, 27 solved;
# the launch's: 100.0, 28) and the main table body (SOURCES, hence Table 6) is built from
# the launch. Pointing this appendix's "after" at load_split instead would print a
# different number for the same model/method one table apart. GPT-5.4 has no launch
# directory (results_validation_split_tool_launch/gpt-5.4 does not exist) so its "after"
# stays load_split, the only split-tool run it has. This also means gpt-4o-mini's and
# sol's PFAgent "after" carries the live V6/V7 gate (the launch ran under it) while
# load_split predates V6/V7 -- so on those two models the PFAgent row's "after" side
# changes tool set AND gate together, a confound the appendix caption must name (see
# build_split_tool_appendix.py). ReAct has no gate, so it is unconfounded on every model.
SPLIT_TOOL_SOURCES: dict[str, dict[str, dict[str, str]]] = {
    ...
    "openrouter:openai/gpt-4o-mini": {
        "pfagent": {"before": "results_validation_gpt-4o-mini_agents", "after": "results_validation_split_tool_launch/gpt-4o-mini/pfagent/case14"},
        "react_nogate": {"before": "results_matrix_gpt-4o-mini/react_nogate/case14", "after": "results_validation_split_tool_launch/gpt-4o-mini/react_nogate/case14"},
    },
    "openrouter:openai/gpt-5.6-sol": {
        "pfagent": {"before": f"{SOL_BASE}/pfagent/case14", "after": "results_validation_split_tool_launch/gpt-5.6-sol/pfagent/case14"},
        "react_nogate": {"before": f"{SOL_BASE}/react_nogate/case14", "after": "results_validation_split_tool_launch/gpt-5.6-sol/react_nogate/case14"},
    },
}
