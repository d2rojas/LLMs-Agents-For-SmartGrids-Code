...
Generado el 2026-09-21 15:01 por benchmarks/experiment_report.py a partir de 1 report(s) del directorio llm_only_cot (ruta completa en la sección 1). Prosa en español; identificadores (métodos, métricas, campos) en inglés tal como aparecen en los datos.
...
````
/Users/lzanda/Documents/REVIEW-TUTORIAL-LLMS-AND-AGENTIC-FOR-SMART-GRIDS/03-codigo-casos-estudio/LLMs-Agents-For-SmartGrids-Code-v2/power-flow-agent/benchmarks/results_validation_n40/gpt-5.4/llm_only_cot
````
...
- Fecha de la corrida (mtime de report.json): 2026-09-18 21:32
- Costo total: 0.6488 USD (40 items; costo medio 0.01622 USD/item)
- Items totales: 40; ok=40; con error=0; tokens totales=116437
- Solved global: 15.0 % (6/40)
...
|---------------|------|----|-----|---------------|---------|----------------|----------|-------------|----------|------------------|---------|------|--------|-----|----------|-----------------|------|
| llm_only:cot | case14 | 40 | -- | 1.18e-03 | 1.18e-03 | -- | 3.72e+00 | 15.0 (6/40) | 0.0 (0/40) | 85.0 | 0.0 | 2911 | 6.43 | -- | 0.0 (0/40) | 85.0 (34/40) | 0.6488 |
...
| Method | plain | parameterized | multistep | ambiguous |
|---------------|-------------|----------------|-------------|-------------|
| llm_only:cot | 40.0 (4/10) | 0.0 (0/10) | 10.0 (1/10) | 10.0 (1/10) |
...
--- Respuesta final ---
Bus ids are the IEEE display ids 1 through 14 from the bus name column. Bus 1 is the slack bus with fixed voltage magnitude 1.06 per unit and angle 0 degrees. Generator buses are 2, 3, 6, and 8 with regulated voltage magnitudes 1.045, 1.01, 1.07, and 1.09 per unit. Load buses are 2, 3, 4, 5, 6, 9, 10, 11, 12, 13, and 14. The requested change is at bus 6, where the active load is set to 14.700 MW instead of the listed 10.690739 MW. Other loads remain unchanged.

Branches include all lines with line ids equal to the net.line row index: 0 is 1-2, 1 is 1-5, 2 is 2-3, 3 is 2-4, 4 is 2-5, 5 is 3-4, 6 is 4-5, 7 is 6-11, 8 is 6-12, 9 is 6-13, 10 is 9-10, 11 is 9-14, 12 is 10-11, 13 is 12-13, 14 is 13-14. Transformers use ids 100000 plus transformer row index: 100000 is 4-7, 100001 is 4-9, 100002 i …[3170 chars más]
--- Scoring ---
formulation=n/a (exact=None); V_MAE=4.61e-04; F_MAE=2.20; faithful=0.0 %; solved=True (numeric_and_answer_ok); flags=-; error=-; tokens=3740; cost_usd=0.0288; wall=11.8 s
````
...
(sin rondas de herramientas: una sola llamada al LLM)
--- Respuesta final ---
Bus ids are the bus name values 1 through 14. Slack bus is bus 1 with fixed 1.06 p.u. and 0 degrees. PV generator buses are 2, 3, 6, and 8 with specified voltage magnitudes. Load buses are 4, 5, 9, 10, 11, 12, 13, 14 and buses 2, 3, 6 also have net demand after local generation. Bus 7 has no direct load or generation.

The disconnected branch is the line from bus 10 to bus 11, which is line table row 12, so line_id 12 is opened and carries zero flow. Other line ids are line row indices 0 to 14. Transformer branch ids are 100000 to 100004 corresponding to transformer table rows 0 to 4.

Requested quantities are bus voltage magnitude in p.u. and angle in degrees, branch active power at the from or hv side in MW, and loading percent. Since thermal ratings are undefined, loading_percent is set …[720 chars más]
--- Scoring ---
formulation=n/a (exact=None); V_MAE=--; F_MAE=--; faithful=0.0 %; solved=False (convergence_mismatch); flags=abstained,abstained_on_solvable; error=-; tokens=2583; cost_usd=0.0115; wall=5.0 s
````
...
(sin rondas de herramientas: una sola llamada al LLM)
--- Respuesta final ---
Bus ids are the net.bus name values 1 through 14. Slack bus is bus 1 with fixed 1.06 p.u. and 0 deg. PV generator buses are 2, 3, 6, and 8 with specified active power and voltage setpoints, subject to reactive limits. Load buses are 4, 5, 9, 10, 11, 12, 13, 14 and also buses 2, 3, 6 carry net load in addition to generation. Bus 7 has no direct load or generation.

Branches include 15 lines with line_id equal to the net.line row index 0 to 14, and 5 transformers with line_id equal to 100000 plus transformer row index, so 100000 to 100004. The requested outage is the line between buses 6 and 13, which is net.line row 9. That branch is disconnected; all other listed lines and transformers remain in service.

Requested quantities are bus voltage magnitude in per unit and angle in degrees; bran …[838 chars más]
--- Scoring ---
formulation=n/a (exact=None); V_MAE=--; F_MAE=--; faithful=0.0 %; solved=False (convergence_mismatch); flags=abstained,abstained_on_solvable; error=-; tokens=2632; cost_usd=0.0121; wall=5.7 s
````
...
- LLM-only: V_MAE entre 1.18e-03 (llm_only:cot/case14) y 1.18e-03 (llm_only:cot/case14) p.u. sobre los items que devolvieron números.
- 30.0 % (12/40) de abstención en LLM-only (respuesta con `converged=false` o arrays vacíos).
- En LLM-only, 23 items no resueltos por `convergence_mismatch` y 3 por `voltage_error`.
- Ningún item terminó con `error`.
- Solved de `llm_only:cot`: 15.0 % (6/40).
- Ningún item reclamó éxito sobre un fallo (claimed_success_on_failure=0).
- Trazabilidad de números (Faith.) media en LLM-only: 4.5 % (sin tools, todo número es no trazable salvo los copiados de la request).
- Costo total 0.6488 USD; el método más caro fue `llm_only:cot` con 0.6488 USD (100 %).
...
````
report: /Users/lzanda/Documents/REVIEW-TUTORIAL-LLMS-AND-AGENTIC-FOR-SMART-GRIDS/03-codigo-casos-estudio/LLMs-Agents-For-SmartGrids-Code-v2/power-flow-agent/benchmarks/results_validation_n40/gpt-5.4/llm_only_cot/report.json   [leído: report.rescored.json]
  scoreboard.json: /Users/lzanda/Documents/REVIEW-TUTORIAL-LLMS-AND-AGENTIC-FOR-SMART-GRIDS/03-codigo-casos-estudio/LLMs-Agents-For-SmartGrids-Code-v2/power-flow-agent/benchmarks/results_validation_n40/gpt-5.4/llm_only_cot/scoreboard.json
  scoreboard.csv: /Users/lzanda/Documents/REVIEW-TUTORIAL-LLMS-AND-AGENTIC-FOR-SMART-GRIDS/03-codigo-casos-estudio/LLMs-Agents-For-SmartGrids-Code-v2/power-flow-agent/benchmarks/results_validation_n40/gpt-5.4/llm_only_cot/scoreboard.csv
  scoreboard.md: /Users/lzanda/Documents/REVIEW-TUTORIAL-LLMS-AND-AGENTIC-FOR-SMART-GRIDS/03-codigo-casos-estudio/LLMs-Agents-For-SmartGrids-Code-v2/power-flow-agent/benchmarks/results_validation_n40/gpt-5.4/llm_only_cot/scoreboard.md
  scoreboard_per_case.json: /Users/lzanda/Documents/REVIEW-TUTORIAL-LLMS-AND-AGENTIC-FOR-SMART-GRIDS/03-codigo-casos-estudio/LLMs-Agents-For-SmartGrids-Code-v2/power-flow-agent/benchmarks/results_validation_n40/gpt-5.4/llm_only_cot/scoreboard_per_case.json
  scoreboard.rescored.json: /Users/lzanda/Documents/REVIEW-TUTORIAL-LLMS-AND-AGENTIC-FOR-SMART-GRIDS/03-codigo-casos-estudio/LLMs-Agents-For-SmartGrids-Code-v2/power-flow-agent/benchmarks/results_validation_n40/gpt-5.4/llm_only_cot/scoreboard.rescored.json
  scoreboard_per_case.rescored.json: /Users/lzanda/Documents/REVIEW-TUTORIAL-LLMS-AND-AGENTIC-FOR-SMART-GRIDS/03-codigo-casos-estudio/LLMs-Agents-For-SmartGrids-Code-v2/power-flow-agent/benchmarks/results_validation_n40/gpt-5.4/llm_only_cot/scoreboard_per_case.rescored.json
  traces (40 archivos): /Users/lzanda/Documents/REVIEW-TUTORIAL-LLMS-AND-AGENTIC-FOR-SMART-GRIDS/03-codigo-casos-estudio/LLMs-Agents-For-SmartGrids-Code-v2/power-flow-agent/benchmarks/results_validation_n40/gpt-5.4/llm_only_cot/traces
````
