...
Generado el 2026-09-18 21:59 por benchmarks/experiment_report.py a partir de 1 report(s) del directorio single_call_structured (ruta completa en la sección 1). Prosa en español; identificadores (métodos, métricas, campos) en inglés tal como aparecen en los datos.
...
````
/Users/lzanda/Documents/REVIEW-TUTORIAL-LLMS-AND-AGENTIC-FOR-SMART-GRIDS/03-codigo-casos-estudio/LLMs-Agents-For-SmartGrids-Code-v2/power-flow-agent/benchmarks/results_validation_n10/gpt-5.6-sol/single_call_structured
````
...
- System prompt: hash `23e5d9b407f9` (10 filas)
- runs por item: 1; k (perturbación ±10 %·k): 1; seeds: [0]; max_rounds: 8; temperature: 0.0; timeout_s: 90.0
- Fecha de la corrida (mtime de report.json): 2026-09-18 04:11
- Costo total: 0.1161 USD (10 items; costo medio 0.01161 USD/item)
- Items totales: 10; ok=10; con error=0; tokens totales=47832
- Solved global: 10.0 % (1/10)
...
|---------------------------|------|----|-------------|-----|-----|-------------|----------------|--------|----------------|----|-----|----|-----|----------|-----------------|------|
| single_call:structured | case14 | 10 | 10.0 (1/10) | 0 | 2.60 | 10.0 (1/10) | 100.0 (10/10) | 2.28e-02 | 100.0 (10/10) | -- | 1.0 | 4783 | -- | 0.0 (0/10) | 90.0 (9/10) | 0.1161 |

### Formulación exacta por dificultad (items con etapa de tools; `n/a` = sin tools)

| Method | plain | parameterized | multistep | ambiguous |
|---------------------------|----------|----------------|---------|---------|
| single_call:structured | 33.3 (1/3) | 0.0 (0/3) | 0.0 (0/2) | 0.0 (0/2) |
...
--- Ronda 2 [final] ---
LLM: The case14 network loaded successfully, but no power-flow result was returned, so the lowest-voltage bus cannot be identified.

**Key numbers**
- Total load: **252.6562386075145 MW**
- Total generation: **Not available** (only generation capacity of **772.4000000000001 MW** was reported)
- Total los …[223 chars más]
--- Respuesta final ---
The case14 network loaded successfully, but no power-flow result was returned, so the lowest-voltage bus cannot be identified.
...
**Violations:** Cannot be assessed without a converged power flow.

**Suggested next step:** Run `run_powerflow()` and inspect the resulting bus voltage magnitudes.
--- Scoring ---
formulation=ok (exact=True); V_MAE=0; F_MAE=0.00; faithful=100.0 %; solved=True (numeric_ok); flags=-; error=-; tokens=4771; cost_usd=0.0117; wall=12.9 s
````
...
tool: load_case({"case_name": "case14"})
  ...
  gate: n/a (no es un resultado de flujo)
--- Ronda 2 [final] ---
LLM: Case14 was loaded successfully, but the requested bus 10–11 line disconnection was not performed.

- **Total load:** 253.84578240591338 MW  
- **Total generation capacity:** 772.4000000000001 MW  
- **Total generation, losses, voltage range:** Not available because no power flow was run  
- **Violat …[121 chars más]
--- Respuesta final ---
Case14 was loaded successfully, but the requested bus 10–11 line disconnection was not performed.

- **Total load:** 253.84578240591338 MW  
- **Total generation capacity:** 772.4000000000001 MW  
- **Total generation, losses, voltage range:** Not available because no power flow was run  
- **Violations:** Not assessed  

**Suggested next step:** Disconnect the branch between buses 10 and 11 and re-run the power flow.
--- Scoring ---
formulation=missed_step (exact=False); V_MAE=2.05e-03; F_MAE=1.39; faithful=100.0 %; solved=False (formulation); flags=-; error=-; tokens=4725; cost_usd=0.0113; wall=7.2 s
````
...
tool: load_case({"case_name": "case14"})
  ...
  gate: n/a (no es un resultado de flujo)
--- Ronda 2 [final] ---
LLM: 1. **Summary:** IEEE case14 was loaded successfully, but the requested 6–13 line disconnection was not executed.
2. **Key numbers:** 14 buses, 5 generators, 20 branches, 11 loads; total load **261.902832 MW**; generation capacity **772.4 MW**. Power-flow generation, losses, and voltage range were no …[196 chars más]
--- Respuesta final ---
1. **Summary:** IEEE case14 was loaded successfully, but the requested 6–13 line disconnection was not executed.
2. **Key numbers:** 14 buses, 5 generators, 20 branches, 11 loads; total load **261.902832 MW**; generation capacity **772.4 MW**. Power-flow generation, losses, and voltage range were not reported.
...
--- Scoring ---
formulation=missed_step (exact=False); V_MAE=8.99e-03; F_MAE=3.80; faithful=100.0 %; solved=False (formulation); flags=-; error=-; tokens=4755; cost_usd=0.0114; wall=8.3 s
````
...
- V_MAE >= 1e-4 p.u. en 1 filas método×caso con solver: single_call:structured/case14 (3.35e-03).
- El gate no verificó ningún resultado de flujo (gate_checked=0 en todos los items).
- Ningún item terminó con `error`.
- Solved de `single_call:structured`: 10.0 % (1/10).
- Formulación exacta agregada sobre métodos con tools: 10.0 % (1/10).
- Formulación por dificultad: mejor `plain` 33.3 % (1/3), peor `parameterized` 0.0 % (0/3).
- Errores de formulación más frecuentes: `missed_step`=9.
- Ningún item reclamó éxito sobre un fallo (claimed_success_on_failure=0).
- Trazabilidad de números (Faith.) media con tools: 100.0 %.
- Costo total 0.1161 USD; el método más caro fue `single_call:structured` con 0.1161 USD (100 %).
...
````
report: /Users/lzanda/Documents/REVIEW-TUTORIAL-LLMS-AND-AGENTIC-FOR-SMART-GRIDS/03-codigo-casos-estudio/LLMs-Agents-For-SmartGrids-Code-v2/power-flow-agent/benchmarks/results_validation_n10/gpt-5.6-sol/single_call_structured/report.json
  scoreboard.json: /Users/lzanda/Documents/REVIEW-TUTORIAL-LLMS-AND-AGENTIC-FOR-SMART-GRIDS/03-codigo-casos-estudio/LLMs-Agents-For-SmartGrids-Code-v2/power-flow-agent/benchmarks/results_validation_n10/gpt-5.6-sol/single_call_structured/scoreboard.json
  scoreboard.csv: /Users/lzanda/Documents/REVIEW-TUTORIAL-LLMS-AND-AGENTIC-FOR-SMART-GRIDS/03-codigo-casos-estudio/LLMs-Agents-For-SmartGrids-Code-v2/power-flow-agent/benchmarks/results_validation_n10/gpt-5.6-sol/single_call_structured/scoreboard.csv
  scoreboard.md: /Users/lzanda/Documents/REVIEW-TUTORIAL-LLMS-AND-AGENTIC-FOR-SMART-GRIDS/03-codigo-casos-estudio/LLMs-Agents-For-SmartGrids-Code-v2/power-flow-agent/benchmarks/results_validation_n10/gpt-5.6-sol/single_call_structured/scoreboard.md
  scoreboard_per_case.json: /Users/lzanda/Documents/REVIEW-TUTORIAL-LLMS-AND-AGENTIC-FOR-SMART-GRIDS/03-codigo-casos-estudio/LLMs-Agents-For-SmartGrids-Code-v2/power-flow-agent/benchmarks/results_validation_n10/gpt-5.6-sol/single_call_structured/scoreboard_per_case.json
  traces (10 archivos): /Users/lzanda/Documents/REVIEW-TUTORIAL-LLMS-AND-AGENTIC-FOR-SMART-GRIDS/03-codigo-casos-estudio/LLMs-Agents-For-SmartGrids-Code-v2/power-flow-agent/benchmarks/results_validation_n10/gpt-5.6-sol/single_call_structured/traces
````
