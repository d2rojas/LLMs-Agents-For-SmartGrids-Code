...
Generado el 2026-09-21 08:32 por benchmarks/experiment_report.py a partir de 1 report(s) del directorio case14 (ruta completa en la sección 1). Prosa en español; identificadores (métodos, métricas, campos) en inglés tal como aparecen en los datos.
...
````
/Users/lzanda/Documents/REVIEW-TUTORIAL-LLMS-AND-AGENTIC-FOR-SMART-GRIDS/03-codigo-casos-estudio/LLMs-Agents-For-SmartGrids-Code-v2/power-flow-agent/benchmarks/results_matrix_gpt-4o-mini/plan_act_nogate/case14
````
...
- Fecha de la corrida (mtime de report.json): 2026-09-14 01:19
- Costo total: 0.0382 USD (40 items; costo medio 0.00096 USD/item)
- Items totales: 40; ok=40; con error=0; tokens totales=229503
...
--- Scoring ---
formulation=ok (exact=True); V_MAE=0; F_MAE=0.00; faithful=100.0 %; solved=True (numeric_and_answer_ok); flags=-; error=-; tokens=3613; cost_usd=0.0006; wall=4.5 s
````
...
executed_calls: load_case({"case_name": "case14"}) -> modify_load({"bus_id": 14, "p_mw": 17.2, "q_mvar": 0}) -> disconnect_line({"from_bus": 1, "to_bus": 5}) -> reconnect_line({"from_bus": 1, "to_bus": 5}) -> run_n1_contingency({"top_k": 8, "criteria": "min voltage", "max_candidates": 100})
fuente de la traza: archivo de traza completo
--- Ronda 1 [plan] ---
LLM: {"plan":[{"tool":"load_case","args":{"case_name":"case14"}},{"tool":"modify_load","args":{"bus_id":14,"p_mw":17.2,"q_mvar":0}},{"tool":"disconnect_line","args":{"from_bus":1,"to_bus":5}},{"tool":"reconnect_line","args":{"from_bus":1,"to_bus":5}},{"tool":"run_n1_contingency","args":{"top_k":8,"criter …[42 chars más]
--- Ronda 2 [act] ---
tool: load_case({"case_name": "case14"})
...
tool: reconnect_line({"from_bus": 1, "to_bus": 5})
  output: {"case_name": "case14", "converged": true, "bus_voltages": [{"bus_id": 1, "vm_pu": 1.06, "va_deg": 0.0, "is_violation": true, "violation_type": "overvoltage"}, {"bus_id": 2, "vm_pu": 1.045, "va_deg": -5.4514286734574515, "is_violation": false, "violation_type": null}, {"bus_id": 3, "vm_pu": 1.01, "v …[6408 chars más]
  gate: PASS (reasons=-, mismatch=5.92e-09 MW)
tool: run_n1_contingency({"top_k": 8, "criteria": "min voltage", "max_candidates": 100})
  output: {"plot_type": "n1_ranking", "figure_json": "{\"data\":[{\"cliponaxis\":false,\"hoverinfo\":\"text\",\"hovertext\":[\"\\u003cb\\u003eOutage\\u003c\\u002fb\\u003e: 1-5 (line)\\u003cbr\\u003e\\u003cb\\u003eConverged\\u003c\\u002fb\\u003e: True\\u003cbr\\u003e\\u003cb\\u003eVoltage violations\\u003c\\u0 …[19561 chars más]
  gate: n/a (no es un resultado de flujo)
--- Ronda 3 [answer] [final] ---
LLM: The analysis was completed successfully with the following results:

1. The total load is 275.14 MW, total generation is 290.61 MW, and total losses are 15.47 MW. The voltage range is from 1.0056 to 1.07 p.u.
2. There are voltage violations at multiple buses, specifically at buses 1, 6, 7, 8, 9, 10, …[980 chars más]
--- Respuesta final ---
The analysis was completed successfully with the following results:

1. The total load is 275.14 MW, total generation is 290.61 MW, and total losses are 15.47 MW. The voltage range is from 1.0056 to 1.07 p.u.
2. There are voltage violations at multiple buses, specifically at buses 1, 6, 7, 8, 9, 10, 11, 12, and 13, all indicating overvoltage conditions.
3. The N-1 contingency analysis identified the 8 worst outages, ranked by minimum voltage:
   - Outage 1-5 (line): 9 voltage violations, worst voltage 1.0056 p.u., worst loading 2.51%
   - Outage 3-4 (line): 9 voltage violations, worst voltage 1.01 p.u., worst loading 1.71%
   - Outage 5-6 (trafo): 9 voltage violations, worst voltage 1.01 p.u., worst loading 1.69%
   - Outage 13-14 (line): 9 voltage violations, worst voltage 1.01 p.u., wors …[480 chars más]
--- Scoring ---
formulation=extra_arg_changes_result (exact=False); V_MAE=1.62e-03; F_MAE=0.08; faithful=100.0 %; solved=False (formulation); flags=stale_state; error=-; tokens=18135; cost_usd=0.0030; wall=7.5 s
````
...
- Trazabilidad de números (Faith.) media con tools: 98.5 %.
- Costo total 0.0382 USD; el método más caro fue `plan_act_nogate` con 0.0382 USD (100 %).
