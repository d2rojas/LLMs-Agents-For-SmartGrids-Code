...
def test_correct_answer_is_solved_for_tools_and_no_tools() -> None:
    ...
    assert notools["common_outcome"] == "solved" and notools["common_formulation_exact"] is True and notools["common_traceable"] is False
...
def test_wrong_formulation_with_tools_is_wrong_and_aggregate_sums() -> None:
    ...
    assert r["common_formulation_error_type"] == "declared_differs_from_executed"
    # without tools, a declared formulation that misses a step is a formulation error too
    bad_decl = json.loads(ans); bad_decl["formulation"] = [{"tool": "load_case", "args": {"case_name": "case14"}}]
    r2 = score_common(answer_text=json.dumps(bad_decl), truth=truth, truth_answer=None, expected_outcome="converged", reference_solvable=True, trace={}, intended_calls=INTENDED, executed_calls=None, has_tools=False, preloaded_case=None, request_text="x", solver_config=SolverConfig())
    assert r2["common_formulation_exact"] is False and r2["common_reason"] == "formulation"
