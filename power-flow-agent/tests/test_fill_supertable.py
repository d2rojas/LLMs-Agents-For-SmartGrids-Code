...
def test_blocks_values_na_and_missing(results_dir, tmp_path):
    ...
    assert b14[4][1] == "Structured" and b14[4][2:] == ["90.0", "85.0", r"$1.65{\times}10^{-5}$", "0.12", "100.0", "90.0", "100.0", "0.0", "2.0", "10000", "3.0"]
    assert b30[4][2] == "70.0" and b30[4][3] == "75.0" and b30[4][2 + names.index("Calls")] == "1.5" and b30[4][2 + names.index("Tok.")] == "12000"
    ...
    assert b14[8][2] == "60.0" and b30[8][2] == "50.0" and b30[8][3] == "55.0"
...
def test_blocks_standalone_and_tabular_wrappers(results_dir, tmp_path):
    ...
    assert r"\begin{tabular}{ll cc ccc ccc ccc}" in tex and r"\toprule" in tex and r"\bottomrule" in tex
    assert r"\textbf{Setting} & \textbf{Method} & \textbf{Solved} & \textbf{Form.} & $V_{\mathrm{MAE}}$" in tex
    assert r"\cmidrule(lr){3-4}\cmidrule(lr){5-7}\cmidrule(lr){8-10}\cmidrule(lr){11-13}" in tex
...
def test_no_prefer_rescored_and_cases_filter(results_dir, tmp_path):
    ...
    assert _data_rows(tex)[4][2] == "10.0"  # original report's solved_rate
