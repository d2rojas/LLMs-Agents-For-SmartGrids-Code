...
- ``llm_only``: the LLM must answer from the case data in the prompt (no tools).
  The structured variant reuses ``baselines.llm_only.build_baseline_prompt`` so
  results remain comparable with the published LLM-only baseline, and every
  strategy keeps the *same* output-format section so
  ``baselines.llm_only.parse_llm_baseline_json`` keeps working.
- ``single_call``: the LLM receives the tool definitions (``llm.tools.TOOLS``,
...
def build_messages(
    ...
    *,
    ...
    caller passes ``llm.tools.get_openai_tools()`` as the API ``tools`` argument.
    """
