...
Input: a run directory ``D`` laid out by ``run.py`` or ``scripts/migrate_paper_results.py``::
    ...
    D/raw/report.json                 written by evaluation/runner.py
    D/raw/report.rescored.json        optional, written by evaluation/rescore.py; preferred
...
No LLM calls, no API keys. Usage::
    ...
    .venv/bin/python evaluation/postprocess.py results/ieee14/2026-09-21/gpt-5.6-sol/pfagent
    .venv/bin/python evaluation/postprocess.py <dir> --method pfagent --condition stress
"""
...
def render_report(header: Dict[str, Any], rows: List[Dict[str, Any]], agg: Dict[str, Any], scoreboard: Optional[Dict[str, Any]], trace_names: Dict[str, str]) -> str:
    ...
    out.append("")
    out.append(f"Generated {_dt.datetime.now().strftime('%Y-%m-%d %H:%M')} by evaluation/postprocess.py from `{Path(header.get('report_file', '')).name}`. Runs: {n}." + (" Scored by the unified evaluator (v2): every verdict below is read from the answer JSON, the same way for every method." if any(r.get('_unified') for r in rows) else ""))
    out.append("")
    out.append("## Run")
    out.append("")
...
def postprocess(run_dir: Path, *, method: Optional[str] = None, model: Optional[str] = None, case: Optional[str] = None, condition: Optional[str] = None, quiet: bool = False, figures: bool = True) -> Dict[str, Any]:
    ...
    if figures:
        from evaluation import trace_figures
