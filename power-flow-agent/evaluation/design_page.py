...
generated from the code that will run (methods/, llm/, evaluation/requests.py, the dispatcher).

    .venv/bin/python evaluation/design_page.py
"""
...
E = html.escape
CASE, N, SEED, K, ROUNDS, TOOLS = "case14", 20, 0, 1, 8, "load_split"  # N = 20 per system (decided 2026-09-25): 5 per difficulty, the first 20 of the seed-0 set
...
PHASES = [
    ...
    ("2", "Scale, small model", ["case30", "case57", "case118", "case300"], ["gpt-4o-mini"], "The same six methods and the same 20-request shape on 30, 57, 118 and 300 buses with gpt-4o-mini: where prompting breaks as the tables grow, and what long solver outputs cost the agents."),
...
def blocks_html(parts: List[Tuple[str, str]], collapsed: Tuple[str, ...] = ("u_data",), shared: Optional[str] = None) -> str:
    ...
    id='udata-<case>'>) and referenced here, so the scenarios do not repeat a 60 KB table."""
    out = []
...
# The hand-written parts of the page (gate table, scoring definitions, answer fields). tests/test_design_page_claims.py
# ties every claim here to the code it describes.
GATE = [("V1", "converged", "The last power flow the agent ran converged."), ("V2", "balance", "Active-power balance of the reported state holds within tolerance."), ("V3", "no_isolated_buses", "No bus was left isolated by the network changes."), ("V4", "traceability", "Every unit-bearing number in the answer appears in a tool output or in the request."), ("V5", "currency", "Those numbers come from the solve made after the last network change."), ("V6", "argument_grounding", "Every argument of a mutating tool traces to the request or to a prior tool output; a positive max_candidates on the N-1 scan must also come from the request."), ("V7", "claims_from_tools", "The answer's claims match the agent's own last solved state."), ("V8", "request_applied", "Every network change the agent made took effect (no tool error, the load change landed on the bus that was asked) and is reflected in the state the answer came from: a disconnected branch carries no flow there, a reconnected one does."), ("V9", "plausible_state", "Advisory, never enforced: voltages between 0.8 and 1.2 p.u. and losses below 10 % of the load, the glance an engineer gives a solve before trusting it. Recorded on the verdict for the narrative.")]
...
FIELDS = [
    ...
    ("bus_voltages, line_flows, total_generation_mw, total_load_mw, total_loss_mw", "the full network state being reported", "copied from the last solve (tools) or computed by hand (no tools)", "compared bus by bus and branch by branch with the reference → V_MAE, flow MAE, coverage, KCL; every number checked against the trace → Traceable"),
...
"""
    ...
    H.append("<div class='card'><h2>One question, six ways to answer it</h2><p>Every method receives the same 20 natural-language power-flow requests per system, on the same perturbed IEEE test systems (14, 30, 57, 118 and 300 buses), answers with the same JSON object, and is scored by the same evaluator. What changes between methods is only <b>who formulates</b> the solver operations, <b>who computes</b> the numbers, whether the method <b>sees the solver's outputs</b> before answering, and whether a <b>gate</b> checks the answer before it is reported.</p>")
    ...
    # ---------------- run plan
    H.append("<div class='tab' id='tab-plan'><div class='card'><h2>What runs and what it costs</h2><p>Six methods, five systems, two models, 20 requests per system (5 per difficulty), one run each at temperature 0. Three phases, in this order; each phase is checked in the trace viewer before the next one is paid for.</p></div>")
    ...
    H.append(f"<div class='card'><h2>Budget</h2><p>All three phases: about ${grand:.2f}, a lower bound. Phase 1 is the paper's main table; phase 2 is small; phase 3 is dominated by the 300-bus system on the frontier model, and runs last, only as far as phase 2 justifies it.</p><h3>How the estimate is made</h3><p class='muted'>14-bus cells use per-request token means measured on pilot runs of the same methods and models, times the prices in <code>pricing.json</code>. Larger systems scale those means: the prompting prompt grows by the extra case-table tokens, an agent's prompt grows with the solver outputs it reads (a fixed {int(AGENT_FIXED_SHARE * 100)}% for the system prompt, tool schema and request does not), and every answer grows by the extra state it must carry. Reasoning models have cost up to twice the estimate. Every run records its own <code>cost_usd_total</code>.</p><p class='muted'>Output goes to <code>results/&lt;system&gt;/&lt;date&gt;/&lt;model&gt;/&lt;method&gt;/</code>; the results page and the table builder read only from there.</p><p class='muted'>Status 2026-09-25: OpenRouter balance ${BALANCE_USD:.2f}; the EV case study still needs about $10.</p></div></div>")
