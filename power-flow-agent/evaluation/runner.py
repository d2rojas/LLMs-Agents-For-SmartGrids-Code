...
* Perturbation and seeds: every item loads the case, applies
  ``evaluation.requests.perturb_network(net, seed=..., k=...)`` (loads and
...
* Request-driven evaluation (``--requests file.jsonl`` or ``--gen-requests N``):
  each ``evaluation.requests.Request`` is one item with ``intended_calls`` and a
...
* Metrics per item (see ``evaluation/metrics.py``): ``formulation_exact`` and
...
DEFAULT_OUT_DIR = "evaluation/results"
...
EXTENDED_SCOREBOARD_FIELDS = [
    ...
    "n_tool_rounds_mean",
    "wall_time_s_mean",
    # end-to-end verdicts (evaluation/scoring.py); appended so older readers keep working
    "solved_rate",
    "solved_count",
    "solved_total",
    ...
    # methods, which have no formulation step) -- see evaluation.scoring's module
    ...
    # escalated_rate (see evaluation.scoring.escalation_check / module docstring).
    "escalated_rate",
    "escalated_count",
    "escalated_total",
...
def _load_requests_file(path: str) -> list[Any]:
    from evaluation.requests import from_jsonl
...
def build_items(
    ...
    * ``requests``: pre-generated requests (filtered to ``case_name``).
    ...
    if requests is None and not gen_requests:
        ...
        from evaluation.requests import compute_ground_truth
...
def evaluate_item(
    *,
    ...
    # JSON-aware failure reporting and the end-to-end `solved` verdict (evaluation/scoring.py).
    ...
    # precedence for this outcome triple; see evaluation.scoring's module docstring.
    ...
    from evaluation.common_eval import score_common
...
def _aggregate_group_legacy(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    # existing behavior for those methods; see evaluation.scoring's module docstring.
    ...
    # (verification_outcome is not None -- see evaluation.scoring.escalation_check),
    ...
    scoreboard = {
        ...
        # `solved_rate` = answered correctly end to end (evaluation/scoring.py).
        ...
        # See evaluation.scoring's module docstring: unconditioned, this column mixes
        ...
        # escalated can overlap (see evaluation.scoring's module docstring).
...
def run_benchmark(
    *,
    ...
    evaluation/merge_shards.py to reassemble afterward, not for standalone use: a
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser.add_argument("--requests", dest="requests_path", default=None, help="requests .jsonl from evaluation/requests.py")
    ...
    parser.add_argument(
        ...
        "-- see evaluation/merge_shards.py to reassemble the full report afterward.",
    )
    parser.add_argument(
