...
actual failure
    ...
    ``runner.py``) -- or, for methods with a solver state, when the
...
def failure_reporting(
    ...
    *,
    ...
    ``reference_solvable`` (``runner.py``'s reference-converges-and-not-
...
def answer_matches_truth(answer_text: Optional[str], truth_answer: Any) -> Optional[bool]:
    ...
    ``truth_answer`` is ``evaluation.requests.compute_ground_truth(...)["answer"]``:
...
def _spelled_out_number(text: str) -> Optional[float]:
    """Reverse of ``evaluation.requests._num_words`` (its own ``_ONES``/``_TENS``
    ...
    from evaluation.requests import _ONES, _TENS
...
def infer_claim_shape(request_text: Optional[str]) -> Optional[Dict[str, Any]]:
    ...
    templates ``evaluation/requests.py``'s generator itself emits (``op_worst_voltage``,
...
def claims_from_tools_check(
    ...
    (notes/mision_cero_erroneas.md's "(a)"): reuses ``evaluation.requests._answer``,
    ...
    from evaluation.requests import _answer as _derive_answer
