...
def verify_final_answer(
    ...
    *,
    ...
    (``evaluation.metrics.argument_grounding_check``: every argument to a mutating
    tool traces to the request or a prior tool output) and V7
    (``evaluation.scoring.claims_from_tools_check``: the answer's claim matches what
    ...
    purpose: this function is also called from runner.py/rescore.py to compute
