...
# Decimals kept in every tool output the model reads (and the trace stores). The solver keeps full
# precision; only the text is shortened. Errors introduced: <= 5e-7 p.u. on voltages, <= 0.5 W on
# powers, far below the evaluator's tolerances (1e-3 p.u., 1%). It cuts a run_powerflow output by
# about 20% (17-digit floats otherwise) on every system, the same for every method with tools.
ROUND_DECIMALS = {"vm_pu": 6, "va_deg": 3, "va_degree": 3, "loading_percent": 2}
ROUND_SUFFIX = {"_mw": 3, "_mvar": 3, "_mva": 3, "_ka": 4, "_pu": 6, "_kv": 3}
ROUND_DEFAULT = 4


def _decimals_for(key: str) -> int:
    if key in ROUND_DECIMALS:
        return ROUND_DECIMALS[key]
    for suf, d in ROUND_SUFFIX.items():
        if key.endswith(suf):
            return d
    return ROUND_DEFAULT


def round_floats(obj: Any, key: str = "") -> Any:
    """Round every float in a tool output, by key (see ROUND_DECIMALS); strings and ints untouched."""
    if isinstance(obj, float):
        return round(obj, _decimals_for(key))
    if isinstance(obj, dict):
        return {k: round_floats(v, str(k)) for k, v in obj.items()}
    if isinstance(obj, (list, tuple)):
        return [round_floats(v, key) for v in obj]
    return obj
    ...
    def dispatch(self, tool_name: str, arguments: Mapping[str, Any]) -> str:
        ...
        try:
            ...
            if isinstance(out, str):
                return out
            return json.dumps(round_floats(out), ensure_ascii=False, default=str)
